...
sent); neither reference client persists any *server* identity, so reconnect is unprotected in both ecosystems as shipped by them; **the 2026-09-15 harvest pass upgrades Android TV reconnect pinning from hypothetical feasibility to VERIFIED OSS implementation precedent** (`ddagunts/ScreenCast` @ `7e66bbe7` enforces an exact stored certificate on reconnect but keys that pin by host/IP; merged TVgrip PR #2 binds its fingerprint to a persisted TV record). Greenfield adopts the verification mechanism with the stricter rule that the pin belongs to the paired-device record, never the current route/IP. That closes the client-side feasibility question, but certificate stability and target-firmware behavior remain **[HARDWARE-REQUIRED]**. One structural conflict is recorded for decision: **Samsung first-use MITM resistance is not achievable by TOFU alone**, because there is no prior fingerprint at first pairing. Casting PARTIAL (Samsung not via WS API, requires Smart View SDK/DIAL/Google Cast separate validation); voice NOT VERIFIED (Samsung VoiceControl is on-TV Web API, Android remote-manager.ts notes voice not implemented); maintenance risk for Android TV reassessed as reverse-engineered not official public API, kud lib very new June 2026.
- [ ] Resolve Samsung first-use MITM exposure — **OPEN CONFLICT, product-owner decision required (not decidable by more client-library research):** Samsung's client-observable pairing path establishes no cryptographic binding, so **no authenticated TV identity exists at the first connection** and first-use MITM resistance cannot be achieved by trust-on-first-use pinning alone. Viable dispositions are: accept a documented residual first-use risk; use an out-of-band confirmation **only if the same identity value is independently authenticated on the TV/vendor side**; or decline/defer Samsung for V1. A fingerprint displayed only by the phone is circular and is **not** a mitigation. This is a non-negotiable-invariant conflict and is routed to the product owner rather than resolved by relaxing the requirement or by reversing ADR-0005.
