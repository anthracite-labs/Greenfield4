...
- Built-in phone IR is a V1 capability when the Android device exposes suitable hardware (fixed product capability). V1 will use a **small provenance-clean profile manifest**, not a wholesale third-party IR database. Harvest Flipper formats/normalization, but ship only profiles with retained source commit/path/license provenance and hardware/user verification. `probonopd/irdb` is a deferred long-tail fallback whose custom obligations are accepted only if a concrete supported-device gap justifies them; LIRC remains rejected for shipping while its data license is unresolved. See `docs/research/2026-09-15-ir-dataset-second-pass.md`.
...
- built-in phone IR where available as fixed capability; V1 ships only provenance-clean IR profiles tracked to exact source commit/path/license plus hardware/user verification. Flipper datasets are harvested for formats/tooling but not blanket-adopted; IRDB is a deferred fallback for concrete coverage gaps; LIRC remains unapproved for shipping;
...
- [x] Resolve Samsung first-use MITM exposure — **ACCEPTED RESIDUAL RISK (product-owner decision, 2026-09-15):** Samsung remains in V1. The product owner explicitly accepts the documented residual first-use risk in the stock remote pairing path: no authenticated TV identity exists at the first connection, and TOFU cannot protect that first connection. This acceptance does **not** weaken reconnect requirements. Greenfield must capture the TV certificate only after the user-approved pairing flow, persist the paired-device identity, require that identity on subsequent connections, fail closed on unexpected identity change, and require explicit re-pairing rather than silently trusting a replacement certificate. No global certificate-verification bypass or insecure fallback is authorized.
- [x] Finalize V1 IR data-source strategy — **HARVEST/ADOPT/REJECT resolved; per-profile provenance remains an implementation/release check:** Greenfield will not adopt any whole historical IR corpus merely because its top-level repository license is permissive. The official Flipper IRDB is **mixed provenance**: the 2024-07-08 `mi_remote_database` bulk import originally carried explicit AGPL-3.0/Ysard notices, while later parsing removed those comments from many transformed files. The community Flipper-IRDB CC0 policy explicitly excludes commits before `2319685`; only 20 TV paths are first-introduced after that boundary, so it is too small as a sole V1 source. V1 therefore uses a small provenance-clean manifest: post-boundary CC0 or individually traced clean contributions first, hardware-verified before Tested. IRDB @ `11aa5eb3` is **DEFERRED** as a long-tail fallback if a concrete supported-device gap justifies accepting its notification/notice/up-to-three-copy obligations. LIRC remains **REJECTED for V1 shipping** unless explicit data/config permission later appears. See `docs/research/2026-09-15-ir-dataset-second-pass.md`.
- [ ] Run the **decision-critical discovery hardware probes** — **full release matrix retained, NOT EXECUTED:** `docs/research/2026-09-14-hardware-validation-matrix.md` remains the pre-release catalogue and the minimum two-model/firmware rule remains required before broad **Tested** claims. Issue #14 narrows the discovery blocker to device-side facts that can still change a security/product decision: Android TV baseline + deliberately corrupted-Secret verification on two firmware generations (**ATV-17a/17b/17d**), the dominant local prefix-failure loop and trial independence/visibility (**ATV-21/22/23**), whether pairing trials can be initiated unattended (**ATV-26**), and how many independent trials one user-mediated session yields (**ATV-29**). **ATV-24/25 become discovery-critical only if ATV-23 shows the TV actually observes local failures; ATV-27/28 are follow-up probes only if initiation/code-lifetime details are needed to bound the result.** Samsung first-use does not need another hardware experiment to establish the structural absence of authenticated first-use identity; its remaining discovery gate is the product-owner risk disposition. Samsung certificate stability, reconnect pinning, recovery, ordinary pairing/discovery, and the rest of both ecosystems' matrix stay required as implementation/release conformance before shipping support is labelled Tested, not as reasons to keep discovery research open. **No hardware row has been run or passed.**
...
Remaining discovery exit criteria: execute the **decision-critical Android TV hardware probes** above and record the result without generalising beyond the tested firmware; complete human/legal review of the exact V1 protocol use; finalize naming; and review PRODUCT.md before moving to architecture. IRDB acceptance is no longer a discovery blocker: it is a deferred fallback to be considered only if a concrete provenance-clean coverage gap appears. Samsung's first-use residual risk is no longer an open discovery conflict: it was explicitly accepted by the product owner on 2026-09-15, with reconnect pinning/fail-closed requirements preserved. The rest of the full hardware matrix remains a pre-release conformance gate, not a reason to continue discovery literature research. The protocol mechanics and common implementation patterns have now been reconciled in `docs/research/2026-09-15-harvest-adopt-reject.md`; **do not restart broad research for a gap already harvested from credible OSS unless new contradictory primary evidence appears.** Product-owner approval of the ecosystem direction is already recorded on Issue #7. Do not move to architecture until security validation is resolved or explicitly accepted as PARTIAL by the product owner, and legal/terms validation is completed.
